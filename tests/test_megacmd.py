import pytest
import asyncio
from unittest.mock import AsyncMock, MagicMock, patch # Or use mocker from pytest-mock
...
from megatui.mega.megacmd import (
    MegaCmdResponse, MegaItem, MegaFileTypes, MegaSizeUnits,
    MegaLibError, MegaCmdError, LS_REGEXP
)

# --- Tests for MegaItem ---
def test_megaitem_file_creation_size_calculation():
    item = megacmd.MegaItem(
        name="test.txt", parent_path="/remote", size=2048, mtime="01Jan2023 10:00:00",
        ftype=megacmd.MegaFileTypes.FILE, version=1, handle="handle123"
    )
...
def test_megaitem_directory_creation():
    item = megacmd.MegaItem(
        name="folder", parent_path="/", size=0, mtime="02Feb2023 11:00:00",
        ftype=megacmd.MegaFileTypes.DIRECTORY, version=0, handle="handle456"
    )
