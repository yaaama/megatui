...
class MegaNode:
    ...
    def __init__(
        ...
        `path` is the full path of the node.
        `bytes` is the size of node in bytes, or 0 if directory.
        `mtime` is date/time of modification.
        `ftype` is a member of `MegaFileTypes` (file or directory).
        `version` is the version of the node.
        `handle` is the unique handle of the node (generated by MEGA).
        """
        self.name = name
        self.bytes = bytes
        self.mtime = mtime
